...
import { PERLER_PALETTE_FULL } from '@/lib/generatedPalettes'
import { Link } from '@/i18n/routing'
import { ArrowLeft, Pipette, Hash } from 'lucide-react'

export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
    const { locale } = await params
    const t = await getTranslations({ locale, namespace: 'perlerColorPage' })

    return {
        title: t('title'),
        description: t('description'),
        keywords: ['Perler beads', 'color chart', 'bead codes', 'pixel art colors', 'perler hex codes'],
        openGraph: {
            title: t('title'),
            description: t('description'),
            type: 'website',
...
export default async function PerlerColorChartPage({ params }: { params: Promise<{ locale: string }> }) {
    const { locale } = await params
    const t = await getTranslations({ locale, namespace: 'perlerColorPage' })
    const colorNames = t.raw('colorNames') as Record<string, string> || {}
    const colors = [...PERLER_PALETTE_FULL]
                ...
                <section className='mt-24 p-8 md:p-12 bg-white border-4 border-[#18181B] shadow-[12px_12px_0px_#18181B] relative overflow-hidden'>
                    ...
                    <div className='prose prose-slate max-w-none text-[#5A4738] font-bold relative z-10'>
                        <p className='text-lg leading-relaxed'>
                            Perler beads (otherwise known as fuse beads or Hama beads) are a fun, inexpensive way to create all sorts of different designs and pixel art.
                            The key to a successful project is choosing the right colors. Our official Perler Bead color chart provides
                            a comprehensive list of every shade, including standard solids, shimmering pearls, and vibrant neons.
                        </p>
                        <div className='mt-8 grid md:grid-cols-2 gap-8'>
                            <div className='p-6 bg-[#F1ECE1] border-2 border-[#18181B]/10'>
                                <h3 className='text-lg font-black uppercase mb-4'>Why Color Mapping Matters</h3>
                                <p className='text-sm'>
                                    Using the correct color codes ensures that your pixel art translates perfectly from digital design to physical craft.
                                    Each color is mapped with its specific HEX code, allowing you to use them in our <Link href="/perler-bead-pattern-generator" className='text-[#EF4444] font-black underline hover:text-[#B91C1C]'>Pixel Bead Generator</Link> for precise pattern creation.
                                </p>
                            </div>
                            <div className='p-6 bg-[#F1ECE1] border-2 border-[#18181B]/10'>
                                <h3 className='text-lg font-black uppercase mb-4'>Mastering Hues</h3>
                                <p className='text-sm'>
                                    Whether you are building complex sprites or simple patterns, having a full range of colors is essential.
                                    We recommend starting with a standard 24 or 48 color set and expanding your collection with individual bags of colors like 01 White, 18 Black, and various flesh tones for characters.
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <script
                ...
                dangerouslySetInnerHTML={{
                    __html: JSON.stringify({
                        ...
                        "@type": "Dataset",
                        "name": "Official Perler Bead Color Chart",
                        "description": "Comprehensive index of Perler Bead colors, codes, and HEX values for pixel art.",
                        "url": `https://www.pixel-beads.com/${locale}/perler-bead-color-chart`,
                        "keywords": ["Perler Beads", "Color Chart", "Pixel Art", "Bead Codes"],
                        "creator": {
                            "@type": "Organization",
                            "name": "PixelBeads"
                        }
